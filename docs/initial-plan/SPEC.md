...
- **Multi-day loop**: iterates 2 simulated days (day 1 seeded, day 2 tomorrow's real partial forecast — see Further Notes on the data-horizon constraint discovered during implementation), threading `last_window_end` forward between iterations, calling the single-day seam each time, collecting `Recommendation`s.
...
- Confirmed live during implementation: Octopus Agile only publishes ~1 day ahead (today fully published, tomorrow partial, nothing beyond). This is why the multi-day loop is 2 days, not the originally-envisioned 3–5 — a longer loop would need to replay real recent past days instead of forecasting further ahead, which remains a future option, not built here.
